...
/*   Updated: 2020/09/30 12:47:41 by kgouacid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		ft_putstr_fd(quotes->q == 0 ? "[ close ]" : "[ open ]", 1);
	}
	else if(!quotes->q && c == '\'')
...
int		ft_count_word(char *str, char *charset)
{
	...
	while (str[i])
	{
		...
		while (ft_is_in_str(str[i], charset) && str[i])
			i++;
		while ((ft_quote_open(&quotes, str[i]) || !ft_is_in_str(str[i], charset)) && str[i])
		{
			nb += (bl ? 0 : 1);
			bl = 1;
			i++;
...
char	*ft_cut_word(char *str, char *charset, int *i)
{
	...
	while (ft_is_in_str(str[*i], charset) && str[*i])
		*i += 1;
	while ((ft_quote_open(&quotes, str[*i + len]) || !ft_is_in_str(str[*i + len], charset)) && str[*i + len])
		len++;
